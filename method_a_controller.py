#classification_9 features:
# === Webots LiDAR Object Classification Controller (Method A) ===
# This script runs a Webots robot, collects LIDAR point cloud data, clusters and classifies objects in real time,
# and visualizes the robot's path, detected objects, and classification results live using matplotlib.
#
# Key features:
...
# - Saves snapshots of raw and clustered LiDAR scans
#
# === Imports ===
...
def update_live_plot(global_points, robot_path, final_objects, classified_objects, door_centers, t):
    plt.clf()
    ax = plt.gca()
    ax.set_title(f"Live LiDAR Classification View")
    ax.set_xlabel("X [m]")
    ax.set_ylabel("Y [m]")
    ax.axis('equal')
    ax.grid(True)
...
while robot.step(timestep) != -1:
    ...
    if len(global_points) > 5000:
        discard_count = int(len(global_points) * 0.05)
        if discard_count > 0:
            global_points = global_points[discard_count:]
        # === ✅ Save snapshot of first raw LiDAR scan (once around t = 1.5s) ===
    if abs(t - 1.5) < 0.05:
        plt.figure(figsize=(6, 6))
        plt.title("Raw 2D LiDAR Scan (First Frame)")
        plt.xlabel("X [m]")
        plt.ylabel("Y [m]")
        plt.axis('equal')
        plt.grid(True)

        raw_rotated = rotate_and_mirror(scan_points)  # Apply same visual alignment
        plt.scatter(raw_rotated[:, 0], raw_rotated[:, 1], s=3, c='black')

        plt.savefig("raw_lidar_scan_snapshot.png", dpi=300)
        print("✅ Raw LiDAR scan snapshot saved as 'raw_lidar_scan_snapshot.png'")
        plt.close()
    ...
    if t - last_cluster_time >= clustering_interval:
        ...
        final_objects, classified_objects, wall_ids, door_centers = run_clustering_and_classification(np.array(global_points))
            # === ✅ Save snapshot of clustered LiDAR points (DBSCAN result only) ===
        if abs(t - 3.0) < 0.05:
            plt.figure(figsize=(6, 6))
            plt.title("Adaptive DBSCAN Clustering of 2D LiDAR Points")
            plt.xlabel("X [m]")
            plt.ylabel("Y [m]")
            plt.axis('equal')
            plt.grid(True)
            ...
            for obj_id, pts in final_objects.items():
                pts_rot = rotate_and_mirror(pts)
                plt.scatter(pts_rot[:, 0], pts_rot[:, 1], s=6, label=f"Cluster {obj_id}")

            plt.legend(fontsize=7, loc='upper right')
            plt.savefig("dbscan_clusters_snapshot.png", dpi=300)
            print("✅ DBSCAN cluster snapshot saved as 'dbscan_clusters_snapshot.png'")
            plt.close()
